...
<head>
    ...
    <style>
        body{background:#f5f7fb}.card{border:0;box-shadow:0 .125rem .5rem rgba(0,0,0,.06)}.qr{max-width:280px;width:100%}.schedule-row.is-locked{background:#f8f9fa}.save-feedback{min-width:92px;display:inline-block}.today-row{box-shadow:inset 4px 0 #0d6efd}.schedule-table td{vertical-align:middle}
    </style>
</head>
<body>
    ...
    <div class="card mb-4" id="jadwal">
        <div class="card-body">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 mb-3">
                <div><h2 class="h5 mb-1">Jadwal Mingguan</h2><div class="text-secondary small"><?=e($weekDate->format('d-m-Y'))?> s/d <?=e($weekEndDate->format('d-m-Y'))?></div></div>
                <button type="button" class="btn btn-outline-primary" id="copyPrevious">Salin Jadwal Minggu Lalu</button>
            </div>

            <div class="d-flex flex-wrap gap-2 mb-3">
                <a class="btn btn-outline-secondary btn-sm" href="?week=<?=e($prevWeek)?>#jadwal">◀ Minggu Sebelumnya</a>
                <a class="btn btn-outline-dark btn-sm" href="?week=<?=e($thisWeek)?>#jadwal">Minggu Ini</a>
                <a class="btn btn-outline-secondary btn-sm" href="?week=<?=e($nextWeek)?>#jadwal">Minggu Berikutnya ▶</a>
            </div>

            <div id="ajaxAlert"></div>
            <div class="table-responsive">
                <table class="table schedule-table align-middle mb-0">
                    <thead><tr><th style="min-width:180px">Tanggal &amp; Hari</th><th style="min-width:180px">Shift</th><th style="min-width:170px">Jam Berangkat</th><th style="min-width:120px">Status</th><th style="min-width:110px">Simpan</th></tr></thead>
                    <tbody>
                    <?php foreach($days as $d): $badge = status_badge($d['status']); ?>
                        <tr class="schedule-row <?= $d['locked']?'is-locked':'' ?> <?= $d['tanggal']===date('Y-m-d')?'today-row':'' ?>" data-date="<?=e($d['tanggal'])?>" data-status="<?=e($d['status'])?>">
                            ...
                            <td>
                                <select class="form-select form-select-sm shift-select" <?= $d['locked']?'disabled':'' ?>>
                                    <option value="">Belum diatur</option>
                                    <?php foreach($shifts as $s): ?><option value="<?=e((string)$s['id'])?>" <?= (string)$d['shift_id']===(string)$s['id']?'selected':'' ?>><?=e($s['kode_shift'].' - '.$s['nama_shift'])?></option><?php endforeach; ?>
                                    <option value="libur" <?= $d['status']==='libur'?'selected':'' ?>>Libur</option>
                                </select>
                            </td>
                            <td><select class="form-select form-select-sm jam-select" data-selected="<?=e($d['jam'])?>" <?= ($d['locked'] || $d['status']==='libur' || !$d['shift_id'])?'disabled':'' ?>><option value="">Pilih jam</option></select></td>
...
const statusMeta = {
    belum_diatur: ['secondary','Belum diatur'],
    terjadwal: ['primary','Terjadwal'],
    terkirim: ['success','Terkirim'],
    libur: ['warning','Libur'],
    gagal: ['danger','Gagal']
...
function fillJam(row, selected = '') {
    const shift = row.querySelector('.shift-select').value;
    const jamSelect = row.querySelector('.jam-select');
    jamSelect.innerHTML = '<option value="">Pilih jam</option>';
    if (!shift || shift === 'libur') { jamSelect.disabled = true; return; }
    (optionMap[shift] || []).forEach(jam => jamSelect.add(new Option(jam, jam)));
    if (selected && (optionMap[shift] || []).includes(selected)) jamSelect.value = selected;
    jamSelect.disabled = row.classList.contains('is-locked') ? true : false;
}

function setStatus(row, status) {
    row.dataset.status = status;
    const badge = row.querySelector('.status-badge');
    const meta = statusMeta[status] || statusMeta.belum_diatur;
    badge.className = `badge status-badge text-bg-${meta[0]}`;
    badge.textContent = meta[1];
}

async function saveRow(row, changedField) {
    const feedback = row.querySelector('.save-feedback');
    const shift = row.querySelector('.shift-select').value;
    const jamSelect = row.querySelector('.jam-select');
    if (changedField === 'shift') {
        if (!shift || shift === 'libur') jamSelect.value = '';
        else if (!(optionMap[shift] || []).includes(jamSelect.value)) jamSelect.value = '';
    }

    feedback.className = 'save-feedback small text-secondary';
    feedback.textContent = 'Menyimpan...';
    const body = new FormData();
    body.append('csrf', csrf);
    body.append('action', 'save');
    body.append('tanggal', row.dataset.date);
    body.append('shift_id', shift);
    body.append('jam_berangkat', jamSelect.value);

    try {
        const response = await fetch('ajax_jadwal.php', {method:'POST', body});
        const data = await response.json();
        if (!data.success) throw new Error(data.message || 'Gagal menyimpan.');
        setStatus(row, data.status);
        feedback.className = 'save-feedback small text-success';
        feedback.textContent = '✓ Tersimpan';
    } catch (error) {
        feedback.className = 'save-feedback small text-danger';
...
function showAlert(message, type='success') {
    document.getElementById('ajaxAlert').innerHTML = `<div class="alert alert-${type} py-2">${message}</div>`;
    setTimeout(() => document.getElementById('ajaxAlert').innerHTML = '', 4000);
}

document.querySelectorAll('.schedule-row').forEach(row => {
    fillJam(row, row.querySelector('.jam-select').dataset.selected || '');
    ...
    shiftSelect.addEventListener('change', async () => {
        fillJam(row, '');
        if (shiftSelect.value === 'libur' || !shiftSelect.value) jamSelect.disabled = true;
        await saveRow(row, 'shift');
    });
    jamSelect.addEventListener('change', () => saveRow(row, 'jam'));
});

document.getElementById('copyPrevious').addEventListener('click', async () => {
    if (!confirm('Salin pola jadwal dari minggu sebelumnya ke minggu ini? Jadwal terkirim yang sudah lampau tidak akan diubah.')) return;
    const button = document.getElementById('copyPrevious');
    button.disabled = true;
    const body = new FormData();
    body.append('csrf', csrf);
    body.append('action', 'copy_previous');
    body.append('week_start', weekStart);
    try {
        const response = await fetch('ajax_jadwal.php', {method:'POST', body});
        const data = await response.json();
        if (!data.success) throw new Error(data.message || 'Gagal menyalin jadwal.');
