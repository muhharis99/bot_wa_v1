...
final class XlsxScheduleReader
{
        ...
        foreach (array_slice($rows, 0, 20, true) as $rowIndex => $row) {
            ...
            foreach ($row as $col => $value) {
                ...
                foreach ($aliases as $key => $names) {
                    if (in_array($label, $names, true)) $map[$key] = $col;
                }
            }
            if (isset($map['tanggal'], $map['shift']) && (isset($map['bulan']) || $this->rowLooksLikeFullDateHeader($row, $map))) {
                return ['row_index' => $rowIndex, 'columns' => $map];
            }
        }
    ...
    public function normalizeSchedule(array $rows, ?int $year = null): array
    {
        ...
        foreach ($dataRows as $excelRow => $row) {
            ...
            if (!$date) {
                $result[] = ['excel_row' => $excelRow + 1, 'valid' => false, 'error' => 'Tanggal tidak dapat dibaca.', 'raw' => $row];
                continue;
            }
            ...
            if ($shift === null) {
                $result[] = ['excel_row' => $excelRow + 1, 'valid' => false, 'error' => "Shift '{$shiftRaw}' tidak dikenali.", 'tanggal' => $date->format('Y-m-d'), 'raw' => $row];
                continue;
            }
            ...
            $result[] = [
                'excel_row' => $excelRow + 1,
                'valid' => true,
                'tanggal' => $dateKey,
                'hari' => $this->dayName((int)$date->format('N')),
                'hari_excel' => $hariExcel,
                'shift' => $shift,
                'jam_berangkat' => $shift === 'L' ? null : $jam,
                'jam_masuk' => isset($header['columns']['jam_masuk']) ? $this->normalizeTime($row[$header['columns']['jam_masuk']] ?? null) : null,
                'warnings' => $warnings,
            ];
        }
    ...
    public function suggestYear(array $rows, array $columns, ?int $centerYear = null): int
    {
        ...
        for ($year = $centerYear - 5; $year <= $centerYear + 5; $year++) {
            ...
            foreach (array_slice($rows, 0, 40, true) as $row) {
                $hari = trim((string)($row[$columns['hari']] ?? ''));
                if ($hari === '') continue;
                $date = $this->resolveDate($row, $columns, $year);
                if (!$date) continue;
                $checked++;
                if ($this->normalizeDay($hari) === $this->normalizeDay($this->dayName((int)$date->format('N')))) $score++;
            }
    ...
    private function firstWorksheetPath(ZipArchive $zip): string
    {
        ...
        if (!$wb || !$rl) return 'xl/worksheets/sheet1.xml';
    ...
    {
        $sx = @simplexml_load_string($xml);
        if (!$sx) throw new RuntimeException('XML worksheet tidak valid.');
        $rows = [];
        foreach ($sx->sheetData->row as $row) {
            ...
            foreach ($row->c as $cell) {
                ...
                if ($type === 'inlineStr') $value = (string)$cell->is->t;
                    ...
                    if ($type === 'n' || ($type === '' && is_numeric($raw))) $value = $raw === '' ? null : (float)$raw;
                }
                $values[$col] = $value;
            }
            if ($values) {
                ...
                $dense = [];
                for ($i = 0; $i <= $max; $i++) $dense[$i] = $values[$i] ?? null;
                $rows[(int)$row['r'] - 1] = $dense;
            }
        }
        return $rows;
    }
    ...
    private function columnIndex(string $ref): int
    {
        ...
        $index = 0;
        foreach (str_split($letters) as $ch) $index = $index * 26 + (ord($ch) - 64);
        return $index - 1;
    }
    ...
    private function resolveDate(array $row, array $columns, int $year): ?DateTimeImmutable
    {
        ...
        if (is_numeric($raw) && (float)$raw > 20000) {
            ...
            return $base->modify('+' . (int)floor((float)$raw) . ' days');
        }
        if (is_string($raw) && preg_match('/\d{1,4}[-\/]\d{1,2}[-\/]\d{1,4}/', $raw)) {
            try { return new DateTimeImmutable(str_replace('/', '-', $raw)); } catch (Throwable) {}
        }
        $day = (int)$raw;
        $month = isset($columns['bulan']) ? $this->monthNumber((string)($row[$columns['bulan']] ?? '')) : null;
        if ($day < 1 || $day > 31 || !$month) return null;
        $date = DateTimeImmutable::createFromFormat('!Y-n-j', "{$year}-{$month}-{$day}");
        return $date && (int)$date->format('j') === $day && (int)$date->format('n') === $month ? $date : null;
    }
    ...
    private function normalizeTime(mixed $value): ?string
    {
        ...
        if (preg_match('/^(\d{1,2})[:.]([0-5]\d)(?::[0-5]\d)?$/', $text, $m)) {
            $h = (int)$m[1]; if ($h <= 23) return sprintf('%02d:%02d', $h, (int)$m[2]);
        }
        return null;
    }
    ...
    {
        $v = strtoupper($this->normalizeText($value));
        return match ($v) {
            'P', 'PAGI', 'MORNING' => 'P',
            'W', 'SIANG', 'DAY' => 'W',
            'S', 'SORE', 'EVENING' => 'S',
            'L', 'LIBUR', 'OFF', 'HOLIDAY' => 'L',
            default => null,
        };
    }
    ...
    private function monthNumber(string $value): ?int
    {
        ...
        $map = [
            ...
            'agu'=>8,'agt'=>8,'agustus'=>8,'aug'=>8,'august'=>8,'sep'=>9,'sept'=>9,'september'=>9,'okt'=>10,'oct'=>10,'oktober'=>10,'october'=>10,
            'nov'=>11,'november'=>11,'des'=>12,'dec'=>12,'desember'=>12,'december'=>12,
        ];
        if (isset($map[$v])) return $map[$v];
        if (ctype_digit($v) && (int)$v >= 1 && (int)$v <= 12) return (int)$v;
        return null;
    }
    ...
    private function normalizeDay(string $value): string
    {
        $v = $this->normalizeText($value);
        return match ($v) {
            'senin','monday','mon' => 'senin', 'selasa','tuesday','tue' => 'selasa', 'rabu','wednesday','wed' => 'rabu',
            'kamis','thursday','thu' => 'kamis', 'jumad','jumat','jum\'at','friday','fri' => 'jumat', 'sabtu','saturday','sat' => 'sabtu',
            'minggu','ahad','sunday','sun' => 'minggu', default => $v,
        };
    }
